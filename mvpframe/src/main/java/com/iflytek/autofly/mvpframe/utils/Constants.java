package com.iflytek.autofly.mvpframe.utils;


/**
 * @author wgsun
 * @descrbe 全局配置
 * @since 2020/1/3 16:10
 */
public class Constants {

    public static Boolean APP_IS_BACKGROUND = true;

    public static final String URL_KEY = "persist.sys.autofly.web.domain";

    public static final String PROP_DUID = "persist.sys.navi.did";
    public static final String PROP_APPID = "persist.sys.navi.openid";

    public static final int ONLINE_TIME_OUT = 60 * 10;
    public static final int OFFLINE_TIME_OUT_DAY = 60 * 60 * 24;

    public static final String MD5_KEY = "8b1f869c41b9b9f4";

}
