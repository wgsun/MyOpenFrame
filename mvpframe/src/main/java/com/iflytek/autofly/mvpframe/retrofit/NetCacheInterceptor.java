...
import com.iflytek.autofly.mvpframe.utils.SharedPreferencesUtils;
...
public class NetCacheInterceptor implements Interceptor {
    ...
    public static final String TAG = NetCacheInterceptor.class.getSimpleName();
    ...
    public Response intercept(Chain chain) throws IOException {
        LogHelper.d(TAG ,"intercept");
        Request request = chain.request();
        Request.Builder builder1 = request.newBuilder();

        //这里坐了自动解析头部和取值。之前一个项目要用头部的Token字段。我也不知道为什么不用cookie
        //(到时候最好用csdn登录来做)
        String token = (String) SharedPreferencesUtils.getShareString(MyApplication.getContext(), "USER_TOKEN");
        LogHelper.d(TAG, "intercept token =" + token);
        ...
        if (list.size() > 0) {
           // PreferenceUtil.put("USER_TOKEN", list.get(0));
            SharedPreferencesUtils.putShareValue(MyApplication.getContext(), "USER_TOKEN", list.get(0));
        }
        ...
        if (onlineCacheTime != 0) {
            ...
            Response response1 = response.newBuilder()
                    ...
                    .build();
            onlineCacheTime = 0;
            LogHelper.d(TAG, "get data from cache");
            return response1;
        } else {
            //如果没有时间就不缓存
            Response response1 = response.newBuilder()
                    ...
                    .removeHeader("Pragma")
                    .build();
            LogHelper.d(TAG, "get data from net");
            return response1;
        }
//        return response;
    }
